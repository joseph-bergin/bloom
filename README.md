...
Needs **Godot 4.3 or newer** and nothing else — no addons, no build step, no
package manager. The font, the audio and the skill tree are all generated or
committed, so a clone runs as-is.

Get Godot from [godotengine.org/download](https://godotengine.org/download)
(the standard build, not .NET).

### The short way

Open `project.godot` in the Godot editor and press **F5**. The editor imports
the project for you on first open.
...
The import step is not optional on a fresh clone. Godot builds its script
class cache during import, and the autoloads reference types like `Contact`
and `TreeNode` that do not exist until it has. Skipping it fails at launch
with `Could not find type "Contact"`. You only need it once — after that,
`godot` on its own is enough.

If `godot` is not on your PATH, use the binary directly:
...
| | |
|---|---|
| **Mouse** | Aim. The turret fires on its own, but only where you point. |
| **Space** (hold) | Hide. Light to 10%, spawning slows fivefold, sight collapses. |
| **T** | Skill tree — between levels only. |
| **Esc** | Options. |

The turret will not lock, and its shots will not touch, anything outside
your light. Point at empty dark and you miss; the reticle turns red when
there is something in range you are not pointed at.

**The first minute.** You start at level 1 with 12 kills to make before the
...
# sweep any tuning value without editing a file
godot --headless --script res://tools/sim_runner.gd -- --set=HP_TIER_MULT=2.6

# regenerate the 130-node tree and the pixel font from their generators
...
ui/font/    5x8 pixel font, generated by tools/gen_font.py
tools/      sim_runner, gen_tree.py, gen_font.py, dev/Shot
